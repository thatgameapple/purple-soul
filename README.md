...
| `Ctrl+S` | 保存当前文件 |
| `Ctrl+N` | 新建文件 |
| `Ctrl+C` | 复制：有选中文字时复制选中部分，无选中时复制全文 |
| `Ctrl+X` | 剪切选中文字 |
| `Ctrl+V` | 粘贴（从系统剪贴板插入到光标处 / 替换选中文字） |
| `Ctrl+E` | 复制全文（无论光标在哪，整篇复制） |
| `Ctrl+L` | 打开文件列表 / 标签浏览 |
...
> **复制 / 剪切 / 粘贴直通系统剪贴板**：`Ctrl+C` `Ctrl+X` `Ctrl+V` 与 macOS 系统剪贴板互通，可以在 purple-soul 和浏览器、备忘录等任意 app 之间自由复制粘贴。用鼠标拖选或 `Shift+方向键` 选中文字。
>
> 注：复制 / 剪切 / 粘贴目前对接 macOS 剪贴板（`pbcopy` / `pbpaste`）；在 Windows / Linux 上写作、保存、标签等功能照常，仅系统剪贴板互通暂未支持。
