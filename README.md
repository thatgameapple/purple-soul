...
`⌃` 念 **Control**（键盘左下角那个键）。**这个软件里所有快捷键都按 `⌃`，包括复制粘贴。**

> ⚠️ **别按 `⌘C`（Command）**。在 Mac 的「终端」里，`⌘C` 会被终端自己截走——它复制的是"终端屏幕上选中的东西"，而你在 purple-soul 里用鼠标选的是 app 自己的选区，终端看不见。结果就是剪贴板压根没变，你粘出来的还是上一次复制的内容。这不是 bug，是终端应用共同的规矩：`⌘` 归终端，`⌃` 才归应用。

### 一、复制 · 剪切 · 粘贴 · 撤销 → 按 `⌃`（Control）

| 按键 | 作用 |
|------|------|
| `⌃ C` | **复制**。先用鼠标选中要复制的文字再按；如果一个字都没选中，就复制整篇全文（提示会告诉你是哪种） |
...
> - 复制成功会在右下角闪一句 `copied.`（复制全文时是 `copied all.`）。**没看到这句，就是没复制成功**——多半是按成 `⌘C` 了。
> - （Windows / Linux 上复制粘贴不通，`⌃C`/`⌃V` 按了没反应；纯写作、保存、搜索不受影响。）
...
> 为什么全都用 `⌃`（Control）不用 `⌘`？因为在终端里 `⌘` 系列（`⌘S`、`⌘N`、`⌘C`…）会被终端和 Mac 系统先抢走，根本传不到软件里。终端应用一律以 `⌃` 为准。
