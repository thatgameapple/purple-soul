...
#   · onefile 是单个文件：复制不破坏内部结构；运行时自解压到系统临时目录再加载。
#     代价：每次启动要解压约 15-30 秒。
#
# 实测(用户真机双击)：onefile 在 ExFAT 剪辑盘上直接双击就能稳定跑(15-30s)。
# 早期"ExFAT 跑不起来"的结论是错的——那是无终端(</dev/null)后台测 textual 的假阴性，
# textual 必须真终端，双击在「终端」App 里有真 tty 才是真实场景。详见 memory。
#
...
cd "$(dirname "$0")"
echo ""
echo "  purple soul 正在启动……约 15-30 秒，请稍候。"
echo ""
exec "./purple-soul"
CMD
